...
import { operationalDayKey, OPERATIONAL_DAY_CUTOFF_TIME } from '@/lib/datetime'
...
export function getDailyOrderControlSummary(
  ...
  const todays = orders.filter(
    (order) =>
      operationalDayKey(order.createdAt, cutoffTime) === resolvedDayKey && order.status !== 'cancelled'
  )
